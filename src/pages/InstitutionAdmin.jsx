  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-8">
      <header className="mb-7">
        <h1 className="font-heading text-3xl font-black">Workspace Analytics</h1>
      </header>
      {error ? (
        <div className="rounded-2xl border border-amber-500/20 bg-amber-500/5 p-6 text-sm leading-6 text-amber-200">{error}</div>
      ) : (
        <>
          <div className="mb-5 flex flex-wrap items-center gap-3 rounded-2xl border border-primary/20 bg-primary/5 p-5"><div className="grid h-11 w-11 place-items-center rounded-xl bg-primary/10 text-primary"><ShieldCheck size={19} /></div><div><h2 className="font-heading font-bold">{data.institution_name}</h2><p className="text-xs text-muted-foreground">{data.scope === 'institution' ? 'Institution totals' : 'Private workspace totals'} · updated {new Date(data.generated_at).toLocaleString()}</p></div></div>
          <section className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            <div className="rounded-2xl border border-border bg-card p-5"><Users size={16} className="text-primary" /><p className="mt-3 text-3xl font-black">{data.members}</p><p className="text-xs text-muted-foreground">{data.scope === 'institution' ? 'Active members' : 'Workspace owner'}</p></div>
